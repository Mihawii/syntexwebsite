...
const TextPressure = ({
    ...
    const animate = () => {
      if (isHoveringRef.current) {
        ...
        if (containerRef.current) {
          ...
          mouseRef.current.x += (containerCenterX - mouseRef.current.x) / 15;
          mouseRef.current.y += (containerCenterY - mouseRef.current.y) / 15;
        }
      }
  ...
  }, [width, weight, italic, alpha, chars.length, interactionRadius]);
